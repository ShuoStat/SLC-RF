...
fit.fun <- function(X, y, newdata, 
  ...
  # source("./rotationForest.R", local = TRUE)
  # rotRF <-rotationForest(x = X, y = as.factor(y), 
  #                        K = round(sqrt(ncol(X))), 
  #                        L = 10, 
  #                        verbose = TRUE)
  # 
  # pred.rotRF <- predict.rotationForest(rotRF, newdata = newdata)
  ...
  # pred.lcRF <- predict.LCRandomForest(LCRF, newdata = newdata, type = "prob")[,2]
  # b <- Sys.time()
  # pred.lcRF_ridge <- list(mod = pred.lcRF,
  #                         class = as.numeric(pred.lcRF > 0.5),
  #                         time = b - a)
