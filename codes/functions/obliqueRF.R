...
obliqueRF.default<- function(x, y,
    ...
    node_pred_method = predict.obliqueRF
...
findbestsplit <- function(dframe,
    ...
    if ((class(z) == "try-error") || (sum(is.na(z$coef)) > 0) || (length(unique(cls)) == 1) || is.null(z$coef)) {
        jstat = 1
...
predict.obliqueRF <- function(object, newdata, type="response", proximity=F, ...) { 
